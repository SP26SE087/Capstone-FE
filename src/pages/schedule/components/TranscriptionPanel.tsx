...
const TranscriptionPanel: React.FC<TranscriptionPanelProps> = ({ onClose, meetingId, meetingName, onProcessingChange, hideTasks = false }) => {
    ...
    const [summaryPollingAttempts, setSummaryPollingAttempts] = useState(0);
    const [summaryPollingTimedOut, setSummaryPollingTimedOut] = useState(false);
    const summaryPollingRef = useRef<ReturnType<typeof setInterval> | null>(null);
    const summaryPollingAttemptsRef = useRef(0);
    ...
    // ── Summary polling (auto-generated by backend) ──────────────────────────
    const handleSummarize = async (autoTrigger = false) => {
        ...
        const currentSummary = normalizeSummaryText(summary) || normalizeSummaryText(transcription.summary);
        if (currentSummary) {
        ...
        if (transcription.status !== 'Completed') {
            if (!autoTrigger) {
                addToast('Summary is generated automatically after transcription is completed.', 'info');
            }
            return;
        }
        ...
        const pollOnce = async () => {
            summaryPollingAttemptsRef.current += 1;
            setSummaryPollingAttempts(summaryPollingAttemptsRef.current);

            const result = await transcriptionService.getById(transcriptionId);
            if (!result) {
                if (summaryPollingAttemptsRef.current >= 12) {
                    stopSummaryPolling();
                    setSummarizing(false);
                    setSummaryPollingTimedOut(true);
                    if (!autoTrigger) {
                        addToast('Summary generation in progress, check back later.', 'info');
                    }
                }
                return;
            }
            ...
            if (nextSummary) {
                ...
                stopSummaryPolling();
                if (!autoTrigger) {
                    addToast('Summary is ready.', 'success');
                }
                return;
            }
        ...
        await pollOnce();
        if (summaryPollingAttemptsRef.current < 12) {
            summaryPollingRef.current = setInterval(() => {
                pollOnce();
            }, 5000);
        }
    };
    ...
    useEffect(() => {
        ...
        const existingSummary = normalizeSummaryText(summary) || normalizeSummaryText(transcription.summary);
        if (existingSummary) {
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '14px' }} className="custom-scrollbar">
                    ...
                    {activeTab === 'summary' && (
                        <>
                            {summarizing ? (
                                <div style={{ ...sectionStyle, textAlign: 'center', padding: '24px' }}>
                                    ...
                                    <p style={{ fontSize: '0.75rem', marginTop: '6px', color: '#64748b' }}>
                                        Checking summary status ({summaryPollingAttempts}/12)
                                    </p>
                                </div>
                            ) : editingSummary ? (
                            ...
                            ) : summaryPollingTimedOut ? (
                                <div style={{ textAlign: 'center', padding: '40px 20px', color: '#64748b' }}>
                                    <Sparkles size={32} style={{ opacity: 0.2, margin: '0 auto 10px' }} />
                                    <p style={{ fontSize: '0.85rem', fontWeight: 600 }}>Summary generation in progress, check back later.</p>
                                </div>
                            ) : (
