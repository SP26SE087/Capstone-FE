    ...
    return (
        <>
            <div style={{ 
                ...
                {[
                    { label: 'Total Tasks', count: tasks.length, color: 'var(--text-primary)', bg: '#f8fafc' },
                    ...columns.map(col => ({
                        label: col.title,
                        count: getTasksByStatus(col.id).length,
                        color: col.color,
                        bg: `${col.color}10` // 10% opacity
                    }))
                ].map((stat, i) => (
                    <div key={i} style={{
                        flex: '1 1 150px',
                        ...
                        flexDirection: 'column',
                        gap: '4px',
                        border: `1px solid ${i === 0 ? '#e2e8f0' : 'transparent'}`
                    }}>
                        <span style={{ fontSize: '0.7rem', fontWeight: 700, color: stat.color, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                            {stat.label}
                        </span>
                        <span style={{ fontSize: '1.5rem', fontWeight: 800, color: i === 0 ? 'var(--text-primary)' : stat.color }}>
                            {stat.count}
                        </span>
                    </div>
                ))}
